...
map_pred <- ggplot(full_predictions, aes(x = gauge_lon, y = gauge_lat, fill = .pred)) +
  geom_tile() +
  coord_fixed(1.3) +
  scale_fill_viridis_c(option = "C", name = "Predicted q_mean") +
  labs(title = "Predicted Mean Flow Across CONUS") +
  theme_minimal()
...
```{r}
map_resid <- ggplot(full_predictions, aes(x = gauge_lon, y = gauge_lat, fill = residuals)) +
  geom_tile() +
  coord_fixed(1.3) +
  scale_fill_viridis_c(option = "B", name = "Squared Residuals") +
  labs(title = "Residuals of Predictions Across CONUS") +
  theme_minimal()
